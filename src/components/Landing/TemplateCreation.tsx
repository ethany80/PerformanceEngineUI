...
type Props = {
    ...
    fromTemplate: (templateId: string, entities: string[], range1: Dayjs, range2: Dayjs) =>  void;
}
        ...
        <div id='template-creation'>
            ...
            <FormControl fullWidth>
                ...
                <Select
                    ...
                    {props.templates.map((template) => (
                        // There will only be one instance of the record
                        Object.entries(template).map(([id,templateName]) => (
                            <MenuItem key={id} value={id}>{templateName}</MenuItem>
                        ))
                    ))}
                </Select>
            </FormControl>
            ...
            <p>Select the target account(s)/position(s) to use with this template.</p>
                ...
                <Button variant='contained' disabled={!validateFields()} onClick={() => {
                    ...
                    props.fromTemplate(selectedTemplate, entities, range1 as Dayjs, range2 as Dayjs);
                }}>Create</Button>
        </div>
    );
};
